...
class ProductRepositoryTest {
  private val mockWebServer = MockWebServer().apply {
    ...
    fun `Verificar si obtenemos correctamente los productos de la DB`() = runBlocking {
        val products = newsRepository.getAllProductRoom().first()
...
class MockProductDao: ProductDao{
    ...
    private val products = MutableStateFlow<List<Product>>(listOf(product1, product2, product3))
...
val myDispatcher: Dispatcher = object : Dispatcher(){
    override fun dispatch(request: RecordedRequest): MockResponse {
        return when (request.path){
            "/$ENDPOINT_PRODUCTS" -> MockResponse().apply { addResponse("api_products.json") }
